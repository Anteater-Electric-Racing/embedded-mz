...
void threadMain(void *pvParameters) {
    ...
    while (true) {
        ...
        // Serial.print("  |  ");
        // if (MCU_GetMCU2Data().dcMainWireOverVoltFault) Serial.print("DC Over
        // Volt Fault, "); if (MCU_GetMCU2Data().motorPhaseCurrFault)
        // Serial.print("Motor Phase Curr Fault, "); if
        // (MCU_GetMCU2Data().mcuOverHotFault) Serial.print("MCU Over Hot Fault,
        // "); if (MCU_GetMCU2Data().resolverFault) Serial.print("Resolver
        // Fault, "); if (MCU_GetMCU2Data().phaseCurrSensorFault)
        // Serial.print("Phase Curr Sensor Fault, "); if
        // (MCU_GetMCU2Data().motorOverSpdFault) Serial.print("Motor Over Spd
        // Fault, "); if (MCU_GetMCU2Data().drvMotorOverHotFault)
        // Serial.print("Driver Motor Over Hot Fault, "); if
        // (MCU_GetMCU2Data().dcMainWireOverCurrFault) Serial.print("DC Main
        // Wire Over Curr Fault, "); if
        // (MCU_GetMCU2Data().drvMotorOverCoolFault) Serial.print("Driver Motor
        // Over Cool Fault, "); if (MCU_GetMCU2Data().dcLowVoltWarning)
        // Serial.print("DC Low Volt Warning, "); if
        // (MCU_GetMCU2Data().mcu12VLowVoltWarning) Serial.print("MCU 12V Low
        // Volt Warning, "); if (MCU_GetMCU2Data().motorStallFault)
        // Serial.print("Motor Stall Fault, "); if
        // (MCU_GetMCU2Data().motorOpenPhaseFault) Serial.print("Motor Open
        // Phase Fault, ");

        Motor_UpdateMotor(
            torqueDemand, enablePrecharge, enablePower, enableRun,
            enableRegen); // Update motor with the current torque demand
#endif
